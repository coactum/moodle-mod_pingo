...
/**
 * pingo entries search.
 *
 * @package   mod_pingo
 * @copyright 2023 coactum GmbH
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
namespace mod_pingo\search;

defined('MOODLE_INTERNAL') || die();

require_once($CFG->dirroot . '/mod/pingo/lib.php');
...
class entry extends \core_search\base_mod {
    ...
    /**
     * Returns recordset containing required data for indexing pingo entries.
     *
    ...
    public function get_document_recordset($modifiedfrom = 0, \context $context = null) {
        ...
        list ($contextjoin, $contextparams) = $this->get_context_restriction_sql($context, 'pingo', 'm', SQL_PARAMS_NAMED);
        if ($contextjoin === null) {
            return null;
        }

        $sql = "SELECT me.*, m.course
                  FROM {pingo_entries} me
                  JOIN {pingo} m ON m.id = me.pingo
          $contextjoin
    ...
    /**
     * Returns the documents associated with this pingo entry id.
     *
     * @param stdClass $entry pingo entry.
     * @param array $options
     * @return \core_search\document
     */
    public function get_document($entry, $options = array()) {
        /* try {
            $cm = $this->get_cm('pingo', $entry->pingo, $entry->course);
            $context = \context_module::instance($cm->id);
        } catch (\dml_missing_record_exception $ex) {
            // Notify it as we run here as admin, we should see everything.
            debugging('Error retrieving mod_pingo '.$entry->id.' document, not all required data is available: '
                .$ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        } catch (\dml_exception $ex) {
            // Notify it as we run here as admin, we should see everything.
            debugging('Error retrieving mod_pingo' . $entry->id . ' document: ' . $ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        }
    ...
    public function check_access($id) {
        ...
        try {
            ...
            $cminfo = $this->get_cm('pingo', $entry->pingo, $entry->course);
        } catch (\dml_missing_record_exception $ex) {
        ...
        if ($entry->userid != $USER->id && ! has_capability('mod/pingo:manageentries', $cminfo->context)) {
            return \core_search\manager::ACCESS_DENIED;
        }
    ...
    /**
     * Returns the specified pingo entry checking the internal cache.
     *
    ...
    protected function get_entry($entryid) {
        /* global $DB;
        return $DB->get_record_sql("SELECT me.*, m.course FROM {pingo_entries} me
                                      JOIN {pingo} m ON m.id = me.pingo
